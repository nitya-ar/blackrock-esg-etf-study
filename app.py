...
def render_tradeoff_scenarios():
    ...
    # =========================
    # VISUALS (only this part is rebuilt)
    # =========================
    ...
    with ch_right:
        ...
        if not base.empty and not alts.empty:
            ...
            # Coalesce AUM from possible __aum__ / __aum__alt
            ...
            if sel_etf == "All" and J["AUM"].sum() > 0:
                ...
                pts = (
                    J.groupby("Scenario").apply(
                        lambda d: pd.Series({"delta_clean_pp": _aw(d, "delta_clean_pp"),
                                             "TE %": _aw(d, "TE %"),
                                             "AUM": d["AUM"].sum()})
                    ).reset_index()
                )
            ...
            if not pts.empty and pts[["delta_clean_pp","TE %"]].notna().any(axis=None):
                ...
                bubble = (
                    ...
                    .encode(
                        ...
                        size=alt.Size("AUM:Q", title="ETF AUM ($)", scale=alt.Scale(type="sqrt", range=[16, 64])),
                        tooltip=[
                            alt.Tooltip("Scenario:N"),
    ...
    with b_left:
        ...
        if deltas is not None:
            ...
            if need_ok:
                ...
                if not use.empty:
                    ...
                    t_chart = (
                        ...
                        .encode(
                            ...
                            column=alt.Column("Metric:N", title=None, header=alt.Header(labelOrient="bottom"), spacing=26),
                ...
                st.info("Turnover cannot be computed: need 'scenario', 'ETF_Ticker', and 'delta'.")
        else:
                ...
                if not S.empty and S[["ActiveShare_%","%Clean"]].notna().any(axis=None):
                    ...
                    scatter = (
                        ...
                        .encode(
                            ...
                            color=alt.Color("Scenario:N", scale=color_scale, title=None),
                            size=alt.Size("AUM:Q", title="ETF AUM ($)", scale=alt.Scale(type="sqrt", range=[16, 56])),
                            tooltip=[
                                alt.Tooltip("Scenario:N"),
