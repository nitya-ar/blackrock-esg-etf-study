...
def line_trends(df, weighting):
    # expect columns like year, weighting (AUM/EW), clean_pct, controversial_pct, other_pct
...
def ribbon_from_dispersion(df, weighting):
    wcol = pick(df, [lambda s: s in ("weighting","agg","type")])
    ycol = pick(df, [lambda s: s=="year"])
    # choose median & p10/p90 for "Controversial" by default
...
def stacked_year_compare(df, weighting, year_a, year_b):
    # expect tidy rows with columns like year, weighting, cohort (Clean/Controversial/Other), pct
    wcol = pick(df, [lambda s: s in ("weighting","agg","type")])
    ycol = pick(df, [lambda s: s=="year"])
...
with colL:
    weighting = st.radio("Weighting", ["AUM-weighted", "Equal-weighted"], horizontal=True, label_visibility="visible", index=0)
with colR:
...
if agg is not None:
    # compute deltas 2017->2025 for selected weighting
...
# Screen small multiples (optional panel)
st.markdown(
    "<div class='pairhead'>"
...
mov = read_csv(MOVERS)
if mov is not None:
    # expect columns: year_a, year_b, ticker, holding_name, delta_pct (or contribution change), direction
    # filter by selected pair if available
