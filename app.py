...
# ---------------------------
# TAB 3: Tradeoff Scenarios (final layout)
# ---------------------------
def render_tradeoff_scenarios():
    ...
    # ---------- header (unchanged copy block from your current tab above charts) ----------
    ...
    # ---------- scoped styles (modern + consistent headings) ----------
    st.markdown("""
    <style>
      ...
      .chart-card{background:var(--card); border:1px solid var(--border); border-radius:14px; padding:12px 14px; margin-bottom:14px;}
      ...
      .two-col-gap > div{padding-right:6px;}
      .table-title{font-size:14px; font-weight:700; margin-bottom:6px;}
    ...
    with c2:
        ...
        <div class="t3-scn-card">
          <div>
            ...
            <div class="desc">Moderate tilt toward clean holdings while keeping sector neutrality and name caps in check.</div>
          </div>
        </div>""", unsafe_allow_html=True)
    with c3:
        st.markdown("""
        <div class="t3-scn-card">
          <div>
            ...
            <div class="desc">Removes all controversial links; rebalances to stay close to the baseline profile.</div>
          </div>
        </div>""", unsafe_allow_html=True)
    ...
    # ---------- LEFT: (A) Composition  (B) Turnover & Cost ----------
    with left:
        # (A) Scenario composition
        st.markdown('<div class="chart-card"><div class="chart-head"><div class="chart-title">Scenario composition</div><div></div></div>', unsafe_allow_html=True)
        comp_rows = []
        for _, r in KP.iterrows():
        ...
        if not comp_df.empty and comp_df["Value"].notna().any():
            ...
            comp_chart = (
                ...
                .properties(height=320, padding={"left": 8, "right": 8, "top": 0, "bottom": 6})
            )
            st.altair_chart(comp_chart, use_container_width=True)
        else:
        ...
        # (B) Turnover & Cost
        st.markdown('<div class="chart-card"><div class="chart-head"><div class="chart-title">Turnover & Cost</div><div></div></div>', unsafe_allow_html=True)
    ...
    # ---------- RIGHT: (C) Uplift vs TE  (D) ActiveShare vs %Clean  (E) Sector drift ----------
    with right:
        # build helper joins once
        A = X.copy()
        ...
        # (C) Uplift vs TE
        st.markdown('<div class="chart-card"><div class="chart-head"><div class="chart-title">Cleanliness Uplift vs Tracking Error</div><div></div></div>', unsafe_allow_html=True)
        if not base.empty and not alts.empty:
            ...
            if not pts.empty and pts[["delta_clean_pp","TE %"]].notna().any(axis=None):
                ...
                bubble = (
                    ...
                    .properties(height=206, padding={"left": 8, "right": 8, "top": 0, "bottom": 6})
                )
        ...
        # (D) Active Share vs % Clean
        st.markdown('<div class="chart-card"><div class="chart-head"><div class="chart-title">Active Share vs % Clean</div><div></div></div>', unsafe_allow_html=True)
        if as_col:
        ...
        # (E) Sector drift heatmap (single color scale)
        st.markdown('<div class="chart-card"><div class="chart-head"><div class="chart-title">Sector drift vs Baseline</div><div></div></div>', unsafe_allow_html=True)
        ...
        if sector_cols:
            ...
            if heat_df["Drift_pp"].notna().any():
                ...
                heat = (
                    ...
                    .mark_rect()
                    .encode(
                        x=alt.X("Sector:N", title=None, axis=alt.Axis(labelAngle=-24)),
                        y=alt.Y("Scenario:N", title=None, sort=["Pragmatic Tilt","Strict Exclusion"]),
                        color=alt.Color("Drift_pp:Q",
                                        title="Drift (pp)",
    ...
    st.caption("Δ weights are in percentage points (pp); aggregated AUM-weighted when “All” ETFs are selected.")
    t_left, t_right = st.columns(2, gap="large")

    # load position deltas (reuse logic)
    deltas2 = None
    for fn in ("load_scenario_position_deltas", "load_scenario_deltas"):
        try:
    ...
    sel_scn_changes = "Pragmatic Tilt"  # default for initial render
    try:
        sel_scn_changes = st.radio("Scenario", ["Pragmatic Tilt","Strict Exclusion"], horizontal=True, key="t3_changes_scn_tables")
    except Exception:
        pass
    ...
    grp = use.groupby([tk_col, nm_col, sec_col], as_index=False).agg(delta_num=("delta_num","sum"), weight=("ETF_Ticker","size"))
    if sel_etf == "All":
        wsum = use.groupby([use[tk_col], use[nm_col], use[sec_col]])["ETF_AUM_USD"].sum().values
    else:
        wsum = np.ones(len(grp))
