...
if mode == "Dashboard":
    ...
    # Compute intersection cohort (ETFs that exist in both selected start year and 2025)
    ...
    # ---- Combined trend — % Clean and % Controversial (0..30 headroom)
    ...
    with left:
        if scr_tr is not None and not scr_tr.empty:
            ...
            if {"screen_category", "value", year_col}.issubset(d.columns):
                ...
                screen_domain = keep
                screen_range  = [
                    "#16A085",  # Clean200 — teal
                    "#B07D62",  # Prisons  — muted ochre/brown
                    "#5B8E7D",  # Deforestation — sage/evergreen
                    "#C46A6A",  # Fossil Fuel — muted brick
                    "#6C82B6",  # Weapons — desaturated steel blue
                    "#9C88B8",  # Tobacco — soft mauve
                ]
