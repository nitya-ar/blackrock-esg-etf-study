...
COLORS = {
    ...
    "text": "#E7EBF0",
    "muted": "#97A2B0",
    "primary": "#E53935",     # aligns with your screenshots
    "clean": "#0E8F66",
    "contro": "#C63C41",
    "other": "#768397",
}
...
st.markdown(
    ...
    <style>
      ...
      [data-testid="stWidgetLabel"], label, .stCaption, .st-emotion-cache-9ycgxx p {{
        color: var(--text) !important;
        opacity: .9 !important;
      }}

      /* Inputs / selects / multiselects / sliders */
      .stTextInput > div > div,
      .stSelectbox > div > div,
      ...
      input::placeholder, textarea::placeholder {{ color: var(--muted) !important; opacity:.8 !important; }}
      .stMultiSelect [data-baseweb="tag"] {{
        background: #1C2027 !important; color: var(--text) !important; border-color:#1C2027 !important;
      }}

      /* Buttons & download buttons */
      .stButton > button, .stDownloadButton > button {{
        background: #1C2027 !important; color: var(--text) !important; border: 1px solid #2A2F38 !important;
      }}
      .stButton > button:hover, .stDownloadButton > button:hover {{ border-color: var(--primary) !important; }}

      /* Segmented controls (incl. View + AUM/EW) */
      [data-testid="stSegmentedControl"] [role="tablist"] > div > button {{
        background: var(--card) !important;
        color: var(--text) !important;
        border: 1px solid var(--border) !important;
      }}
      [data-testid="stSegmentedControl"] [role="tablist"] > div > button[aria-pressed="true"] {{
        background: var(--primary) !important;
        color: #fff !important;
        border-color: var(--primary) !important;
      }}

      /* Tabs (headers + active underline) */
      .stTabs [data-baseweb="tab"] button, .stTabs [data-baseweb="tab"] p {{
        color: var(--text) !important;
      }}
      ...
      /* DataFrame/table panes (headers, rows, scroll area) */
      ...
      div[data-testid="stDataFrame"] * {{
        color: var(--text) !important; font-family: 'Inter', system-ui, -apple-system, Segoe UI, Roboto, sans-serif !important;
      }}

      /* Markdown tables */
      table, thead th, tbody td {{ background: #0E1015 !important; color: var(--text) !important; border-color: var(--border) !important; }}
      ...
      /* Altair embeds on transparent bg */
      .vega-embed, .vega-embed > div {{ background: transparent !important; }}

      /* ===== Full-screen loading overlay ===== */
      .blx-loading {{
        position: fixed; inset: 0; background: rgba(10,11,13,0.95);
        display: flex; align-items: center; justify-content: center;
        z-index: 9999999; backdrop-filter: blur(1.5px);
      }}
      .blx-spinner {{
        width: 86px; height: 86px; border-radius: 50%;
        border: 6px solid rgba(231,235,240,0.15);
        border-top-color: #fff; animation: blx-spin 0.9s linear infinite;
      }}
      @keyframes blx-spin {{ to {{ transform: rotate(360deg); }} }}
      .blx-loading p {{ color: #E7EBF0; margin-top: 16px; font-weight: 700; letter-spacing: .3px; text-align:center; }}
    </style>
    """,
...
def _blx_transparent_theme():
    return {
        "config": {
            ...
            "axis": {"labelColor": COLORS["text"], "titleColor": COLORS["muted"], "gridColor": COLORS["border"], "tickColor": COLORS["border"]},
...
def divider(): st.markdown('<div style="border-top:1px solid var(--border); margin: 10px 0 24px;"></div>', unsafe_allow_html=True)
def gap(px=6): st.markdown(f'<div style="height:{px}px;"></div>', unsafe_allow_html=True)

# =========================
# ATOMIC RENDERING HELPERS
# =========================
def _show_loader(ph, text="Loading…"):
    ph.markdown(
        f"""
        <div class="blx-loading">
...
def atomic_section(text="Loading…"):
    """
    Show a full-screen overlay immediately, then render the section into a placeholder
    only after all computations are done. Prevents half-painted charts/controls.
    """
    shell = st.empty()
    _show_loader(shell, text)
    try:
        yield lambda: shell.container()
    finally:
        pass  # shell.container() replaces the loader automatically
...
st.markdown(
    ...
    <div style="display:flex; flex-direction:column; gap:8px;">
      ...
      <div style="max-width:1400px; text-align:justify; color: var(--muted);">
...
# -------------------------
# RENDERER: CHANGE SINCE 2017 (atomic)
# -------------------------
def render_change_since_2017():
    with atomic_section("Loading Change since 2017…") as mount:
        # ===== compute first =====
        try:
        ...
        years    = sorted([int(y) for y in by_fund[year_col].dropna().unique().tolist()]) if year_col in by_fund.columns else list(range(2017, 2025+1))
        end_year = 2025 if 2025 in years else (max(years) if years else 2025)
        min_year = min(years) if years else 2017

        df_all = by_fund.copy()

        # ===== now paint everything together =====
        with mount():
            ...
            with topC:
                st.markdown(
                    ...
                    '<div class="info-badge" >i</div></div>',
                    unsafe_allow_html=True,
                )
            ...
            with k1:
                ...
                if tp_net is not None: st.altair_chart(slope_chart(tp_net, [-20, 0], "#8A93A6"), use_container_width=True)
            with k2:
                ...
                if tp is not None: st.altair_chart(slope_chart(tp, [0, 30], COLORS["clean"]), use_container_width=True)
            with k3:
                ...
                if tp is not None: st.altair_chart(slope_chart(tp, [0, 30], COLORS["contro"]), use_container_width=True)
            with k4:
                st.markdown(
                    f"""
                    <div class="kpi" style="display:flex;align-items:center;gap:14px;">
                      <div style="font-size:44px; font-weight:800; line-height:1;">{len(cohort)}</div>
            ...
            gap(8)
            st.markdown('<div class="chart-head"><div class="chart-title">Combined trend — % Clean and % Controversial</div><div class="info-badge">i</div></div>', unsafe_allow_html=True)
            ...
            if not comb.empty:
                ...
                band_layer = alt.Chart(band_df).mark_area(opacity=0.10).encode(
                ...
                line_layer = alt.Chart(comb).mark_line(point=True, clip=True).encode(
                    x=alt.X(f"{year_col}:O", title=None, axis=alt.Axis(labelAngle=0, labelPadding=10)),
                ...
                st.altair_chart(alt.layer(band_layer, line_layer).properties(height=300, padding={"left": 8, "right": 8}), use_container_width=True)
            ...
            with left:
                ...
                if scr_tr is not None and not scr_tr.empty:
                    d = scr_tr.copy().rename(columns={"exposure_pct": "value"})
                    keep = ["Clean200", "Prisons", "Deforestation", "Fossil Fuel", "Weapons", "Tobacco"]
                    if "screen_category" in d.columns:
                        d["Category"] = d["screen_category"].astype(str).str.strip().str.title().replace({"Prison":"Prisons","Fossil_Fuel":"Fossil Fuel","Fossil_fuel":"Fossil Fuel"})
                        d = d[d["Category"].isin(keep)]
                    if "year" in d.columns:
                        d = d[(d["year"] >= start_year) & (d["year"] <= end_year)]
                    chart = alt.Chart(d).mark_line(strokeWidth=2, opacity=0.95, point=alt.OverlayMarkDef(size=36, opacity=0.95)).encode(
                        x=alt.X("year:O", title=None, axis=alt.Axis(labelAngle=0, labelPadding=10)),
                        y=alt.Y("value:Q", title="Exposure (%)", axis=alt.Axis(format=".1f"), scale=alt.Scale(domain=[0, 15])),
                        color=alt.Color("Category:N", title=None, scale=alt.Scale(domain=keep, range=["#5C6ACF","#C97F64","#EDE7DE","#B5A793","#2FA08A","#A99ABD"])),
            ...
            with right:
                ...
                def _series(col):
                    s = df.groupby(year_col)[col].mean().reset_index() if col in df.columns else pd.DataFrame({year_col:[], "value":[]})
                ...
                for label, col in [("Clean", clean_col), ("Controversial", ctr_col), ("Other", oth_col)]:
                    ...
                    if not s.empty:
                        ...
                        if len(vA): rows.append({"Year": str(start_year), "Category": label, "Value": float(vA.iloc[0])})
                        if len(vZ): rows.append({"Year": str(end_year),   "Category": label, "Value": float(vZ.iloc[0])})
            ...
            # Movers: year-filter if columns exist; else fallback to overall top 10
            ...
            if not topm_df.empty:
                cols = {c.lower(): c for c in topm_df.columns}
                name25_col = next((cols[c] for c in cols if "name_2025" in c or "holding_2025" in c), None)
                base_name  = next((cols[c] for c in cols if c in ["holding","name","security_name"]), None)
                cat_col    = next((cols[c] for c in cols if "category" in c or c=="class"), None)
                y0_col     = next((cols[c] for c in cols if "start_year" in c or "year_a" in c or "year0" in c), None)
                y1_col     = next((cols[c] for c in cols if "end_year" in c or "year_b" in c or "year1" in c), None)
                d_col      = next((cols[c] for c in cols if c in ["delta","delta_pp","pp","change"]), None)

                m = topm_df.copy()
                if y0_col and y1_col:
                    m = m[(m[y0_col] == start_year) & (m[y1_col] == 2025)]
                # else: keep all rows as fallback
                ...
                if d_col in m.columns and not m.empty:
                    ...
                    holding_name = m[name25_col] if (name25_col and name25_col in m.columns) else (m[base_name] if base_name in m.columns else pd.Series(["—"] * len(m), index=m.index))
                    movers_view = pd.DataFrame({
                        "Holding (2025)": holding_name,
                        "Category": m[cat_col] if (cat_col and cat_col in m.columns) else "—",
...
if mode == "Dashboard":
    ...
    # ---------- 2025 OVERVIEW ----------
    with tab1:
        with atomic_section("Loading 2025 Overview…") as mount:
            # compute first
            ...
            with mount():
                ...
                with c1:
                    st.markdown("""<div class="chart-head"><div class="chart-title">2025 Composition — Clean vs Controversial vs Other</div><div></div></div>""", unsafe_allow_html=True)
                    if {"classification","share_of_total_aum_pct"}.issubset(ctx.columns):
                        comp = ctx[ctx["classification"].str.lower().isin(["clean","controversial","other"])].copy()
                        comp["classification"] = comp["classification"].str.title()
                        ...
                        chart = alt.Chart(comp).mark_bar(opacity=0.92, stroke='#0A0B0D', strokeWidth=0.6).encode(
                            ...
                            color=alt.Color("classification:N", scale=alt.Scale(domain=["Clean","Controversial","Other"], range=[COLORS["clean"], COLORS["contro"], COLORS["other"]]), legend=alt.Legend(orient="top", title=None)),
                            tooltip=[alt.Tooltip("classification:N"), alt.Tooltip("share_of_total_aum_pct:Q", title="Share (%)", format=".1f")]
                        ).properties(height=120)
                        st.altair_chart(chart, use_container_width=True)

                with c2:
                    st.markdown("""<div class="chart-head"><div class="chart-title">By-screen exposures — share of total AUM</div><div class="info-badge">i</div></div>""", unsafe_allow_html=True)
                    parts, clean200 = [], 0.0
                    ...
                    scr_all = pd.concat(parts, ignore_index=True).groupby("screen_category", as_index=False)["share_of_total_aum_pct"].sum().sort_values("share_of_total_aum_pct", ascending=True)
                    scr_all["color"] = scr_all["screen_category"].apply(lambda x: COLORS["clean"] if str(x).strip().lower()=="clean200" else COLORS["contro"])
                    chart2 = alt.Chart(scr_all).mark_bar(opacity=0.92, stroke='#0A0B0D', strokeWidth=0.6).encode(
                        ...
                        tooltip=[alt.Tooltip("screen_category:N", title="Category"), alt.Tooltip("share_of_total_aum_pct:Q", title="Share (%)", format=".1f")],
                ...
                with s1:
                    ...
                    if "cohort" in spot.columns:
                        ...
                        cont_disp = cont.rename(columns={"rank_within_cohort":"Rank","ticker":"Ticker","holding_name":"Holding","share_of_total_aum_pct":"Share of AUM (%)","num_etfs":"#ETFs","screen_categories":"Screens"})[["Rank","Ticker","Holding","Share of AUM (%)","#ETFs","Screens"]]
                        if "Share of AUM (%)" in cont_disp.columns: cont_disp["Share of AUM (%)"] = pd.to_numeric(cont_disp["Share of AUM (%)"], errors="coerce").map(lambda v: f"{v:.2f}")
                ...
                with s2:
                    ...
                    if "cohort" in spot.columns:
                        ...
                        clean_disp = clean.rename(columns={"rank_within_cohort":"Rank","ticker":"Ticker","holding_name":"Holding","share_of_total_aum_pct":"Share of AUM (%)","num_etfs":"#ETFs","screen_categories":"Screens"})[["Rank","Ticker","Holding","Share of AUM (%)","#ETFs","Screens"]]
                        if "Share of AUM (%)" in clean_disp.columns: clean_disp["Share of AUM (%)"] = pd.to_numeric(clean_disp["Share of AUM (%)"], errors="coerce").map(lambda v: f"{v:.2f}")
                ...
                # mini atomic block for filters compute
                holder = st.empty()
                _show_loader(holder, "Filtering holdings…")
                # compute masks
                ...
                with holder.container():
                    ...
                    with fc1: sel_etfs_explorer = st.multiselect("ETF", etfs, placeholder="All")
                    with fc2: sel_class = st.multiselect("Classification", classes, default=[], placeholder="Any")
                    with fc3: sel_tags = st.multiselect("Screen tags", all_tags, default=[], placeholder="Any")
                    with fc4: sel_sector = st.multiselect("Sector", sectors, default=[], placeholder="Any")
                    with fc5: sel_region = st.multiselect("Region", regions, default=[], placeholder="Any")
                    ...
                    for c in ("Weight % in ETF","ETF AUM (USD)","$ Contribution (Agg)"):
                        if c in df_f.columns: df_f[c] = pd.to_numeric(df_f[c], errors="coerce")
    ...
    # ---------------- Tradeoff Scenarios ----------------
    with tab3:
        with atomic_section("Loading scenarios…") as mount:
            with mount():
                ...
                with c1:
                    ...
                    st.markdown('<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px">Charts coming</div>', unsafe_allow_html=True)
                    gap(10)
                ...
                with c2:
                    ...
                    st.markdown('<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:14px">Table coming</div>', unsafe_allow_html=True)

else:
    # ---------------- REPORT ----------------
    with atomic_section("Building report…") as mount:
        with mount():
...
st.markdown(
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
      <div style="color: var(--muted); font-size: 14px; white-space: nowrap;">Built by <strong>Nitya Arya</strong></div>
      <div class="footer-links" style="display:flex; gap:28px; align-items:center;">
        ...
        <a href="https://forms.gle/qid7S1eJpGCuYdtY8" target="_blank" style="color: var(--text); text-decoration:none; font-size:15.5px; font-weight:600;">Send Feedback</a>
      </div>
    </div>
    """,
    unsafe_allow_html=True,
