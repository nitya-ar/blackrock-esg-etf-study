...
st.markdown(
    ...
    <style>
      ...
      /* Inputs, selects, multiselects, sliders, pills */
      .stTextInput > div > div,
      .stSelectbox > div > div,
      ...
      /* Altair canvases on transparent bg */
      ...
      /* =========================
         ...
         ========================= */
      .blx-loading {{
        position: fixed; inset: 0; background: rgba(10,11,13,0.92);
        display: flex; align-items: center; justify-content: center;
        z-index: 9999999; backdrop-filter: blur(1.5px);
      }}
      .blx-spinner {{
        width: 84px; height: 84px; border-radius: 50%;
        border: 6px solid rgba(231,235,240,0.12);
        border-top-color: #ffffff; animation: blx-spin 1s linear infinite;
      }}
      @keyframes blx-spin {{ to {{ transform: rotate(360deg); }} }}
      .blx-loading p {{ color: #E7EBF0; margin-top: 16px; font-weight: 600; letter-spacing: .4px; text-align:center; }}
    </style>
    """,
    unsafe_allow_html=True,
)

# Altair theme (transparent) — keep
    ...
    return {
        "config": {
            ...
            "axis": {
                ...
                "gridColor": COLORS["border"], "tickColor": COLORS["border"],
            },
...
# =========================
def _show_loader(placeholder, text="Loading…"):
...
def _hide_loader(placeholder):
    placeholder.empty()

@contextlib.contextmanager
def loading_overlay(text="Loading…"):
    holder = st.empty()
    _show_loader(holder, text)
    try:
        yield
    finally:
        _hide_loader(holder)
...
def render_change_since_2017():
    with loading_overlay("Loading Change since 2017…"):
        ...
        # ---- Controls row (each control triggers rerun; overlay wraps full block)
        ...
        if not comb.empty:
            ...
            band_layer = (
                ...
                .encode(
                    x=alt.X(f"{year_col}:O", title=None, axis=alt.Axis(labelAngle=0, labelPadding=10, labelFlush=False)),
            ...
            st.altair_chart(alt.layer(*layers).properties(height=300, padding={"left": 8, "right": 8}),
        ...
        if topm_df is not None and not topm_df.empty:
            name25_col = next((c for c in topm_df.columns if "name_2025" in c.lower() or "holding_2025" in c.lower()), None)
            base_name  = next((c for c in topm_df.columns if "holding" in c.lower() or "name" in c.lower()), None)
            cat_col    = next((c for c in topm_df.columns if "category" in c.lower() or "class" in c.lower()), None)
            y0_col     = next((c for c in topm_df.columns if "start_year" in c.lower() or "year_a" in c.lower() or "year0" in c.lower()), None)
            y1_col     = next((c for c in topm_df.columns if "end_year" in c.lower() or "year_b" in c.lower() or "year1" in c.lower()), None)
            d_col      = next((c for c in topm_df.columns if c.lower() in ["delta","delta_pp","pp","change"]), None)
...
if mode == "Dashboard":
    ...
    # ---------- 2025 OVERVIEW ----------
    with tab1:
        with loading_overlay("Loading 2025 Overview…"):
            ...
            # Explorer section with its own micro overlay during filter recompute
            with loading_overlay("Filtering holdings…"):
                ...
                st.download_button("Download filtered rows (CSV)", data=csv_bytes,
                                   file_name="holdings_explorer_filtered.csv", mime="text/csv")
    ...
    # ---------------- Tradeoff Scenarios ----------------
    with tab3:
        with loading_overlay("Loading scenarios…"):
    ...
    # ---------------- REPORT ----------------
    with loading_overlay("Building report…"):
